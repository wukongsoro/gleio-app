...
// Respect explicit VITE_LOG_LEVEL first; otherwise default to 'info' in dev and prod to reduce noise
let currentLevel: DebugLevel = ((): DebugLevel => {
  const envLevel = (import.meta as any).env?.VITE_LOG_LEVEL as DebugLevel | undefined;
  if (envLevel === 'trace' || envLevel === 'debug' || envLevel === 'info' || envLevel === 'warn' || envLevel === 'error') {
    return envLevel;
  }
  // Be conservative by default; developers can opt into more verbose levels via VITE_LOG_LEVEL
  return 'info';
})();
